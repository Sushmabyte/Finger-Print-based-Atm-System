...
def LoginAction():
    ...
    if request.method == 'POST':
        ...
        index = 0
        con = pymysql.connect(host='127.0.1.1',port = 3306,user = 'root', password = 'root', database = 'atm',charset='utf8')
        with con:
            cur = con.cursor()
            ...
            for row in rows:
                if row[0] == user and password == row[1]:
                    in_file = open("C:\Users\DeLL\Desktop\S-4 B-2\Finger print based atm"+user+".png", "rb")
...
def SignupAction():
    if request.method == 'POST':
        ...
        if status == 'none':
            ...
            db_connection.commit()
            if db_cursor.rowcount == 1:
                out_file = open("C:/Finger print based atm/FingerprintATM/static/users/"+user+".png", "wb")
                out_file.write(data)
                out_file.close()
